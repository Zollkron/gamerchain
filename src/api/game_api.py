...
class GameAPI:
    ...
    def _setup_routes(self):
        ...
        self.app.route('/api/v1/balance/<address>', methods=['GET'])(
            self.limiter.limit("60 per minute")(self.require_auth(self.get_balance))
        )
        self.app.route('/api/v1/transaction', methods=['POST'])(
            self.limiter.limit("30 per minute")(self.require_auth(self.create_transaction))
        )
        self.app.route('/api/v1/transaction/<tx_hash>', methods=['GET'])(
            self.limiter.limit("60 per minute")(self.require_auth(self.get_transaction))
        )
        self.app.route('/api/v1/transactions/history/<address>', methods=['GET'])(
            self.limiter.limit("30 per minute")(self.require_auth(self.get_transaction_history))
        )
